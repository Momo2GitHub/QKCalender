    ...
    public class QRCodeEncoder
    {
		...
		public virtual bool[][] calQrcode(byte[] qrcodeData)
		{
			...
			try
			{
                ...
                SystemUtils.ReadInput(reader.BaseStream, matrixX, 0, matrixX.Length);
                ...
                SystemUtils.ReadInput(reader.BaseStream, rsBlockOrderTemp, 0, rsBlockOrderTemp.Length);
			...
			try
			{
				...
				//BufferedStream bis = new BufferedStream(memoryStream);
                SystemUtils.ReadInput(reader.BaseStream, frameData, 0, frameData.Length);
				//bis.Close();
                //memoryStream.Close();
                reader.Close();
                //fis.Close();
		...
		private static sbyte[] calculateRSECC(sbyte[] codewords, sbyte rsEccCodewords, sbyte[] rsBlockOrder, int maxDataCodewords, int maxCodewords)
		{
			...
			try
			{
				...
				for (int i = 0; i < 256; i++)
				{
                    SystemUtils.ReadInput(reader.BaseStream, rsCalTableArray[i], 0, rsCalTableArray[i].Length);
				}
				//bis.Close();
				reader.Close();
                //memoryStream.Close();
